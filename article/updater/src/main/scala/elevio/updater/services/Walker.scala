...
object ElevioWalker {
  case class ElevioWalkerConfig(maxItemPar: Int)
  def apply(
  ...
  )(implicit cs: ContextShift[IO]): Walker = new Walker {
    override def run: IO[Unit] =
      Stream
        ...
        .map(_._1)
        .evalMap(article => updatePublisher(ArticleUpdate(article.id)))
        .compile
        .drain
  }
}
...
object InternalWalker {
  case class InternalWalkerConfig(maxItemPar: Int, itemsPerPage: ItemsPerPage)
  ...
  def apply(
    ...
    override def run: IO[Unit] =
      Stream
        ...
        .takeThrough(p => p.page.value < p.totalPages.value)
        .flatMap(page => Stream.emits(page.elements))
        .parEvalMap(config.maxItemPar)(internal =>
          for {
